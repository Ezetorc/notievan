...
describe('CommentsService', () => {
  ...
  describe('create', () => {
    it('should create a new comment', async () => {
      ...
      const result = await CommentsService.create(content, articleId, userMock.id)
    ...
    it("should throw a NotFound error because comment's article does not exist", async () => {
      ...
      await expect(CommentsService.create(content, articleId, userMock.id)).rejects.toBeInstanceOf(NotFoundError)
      expect(ArticlesService.exists).toHaveBeenCalledWith(articleId)
    })
  })
  ...
  describe('getAllOfArticle', () => {
    it('should return all comments of an article', async () => {
      ...
      vi.spyOn(CommentsRepository, 'getAllOfArticle').mockResolvedValue(comments)

      const result = await CommentsService.getAllOfArticle(articleId, limit)

      expect(result).toHaveProperty("data", comments)
      expect(CommentsRepository.getAllOfArticle).toHaveBeenCalledWith(articleId, limit, undefined)
    })
  })
  ...
  describe('delete', () => {
    ...
    it("should throw a NotFound error if comment doesn't exist", async () => {
      ...
      await expect(CommentsService.delete(commentMock.id, userMock.id)).rejects.toThrow(NotFoundError)
    })
    ...
    it("should throw an Unauthorized error if user is not the author", async () => {
      ...
      await expect(CommentsService.delete(commentMock.id, "different-user-id")).rejects.toThrow(UnauthorizedError)
    })
  })
})
