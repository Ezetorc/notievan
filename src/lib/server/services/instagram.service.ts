...
import type { Token } from 'tokens/server/models/token.model'
import { TokensService } from 'tokens/server/services/tokens.service'
...
export class InstagramService {
	...
	private static async request<T>({
		...
		try {
			...
			throw error(500, ErrorCode.INSTAGRAM_ERROR)
		}
	...
	static async refreshAccessToken(): Promise<void> {
		...
		if (!response.ok) {
			...
			console.error('[Instagram] Refresh token error:', err)
